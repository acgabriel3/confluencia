...
ligada e não é válido para nossa representação.
 ...
 Assim podemos definir o conceito de pré-termo como sendo a gramática contendo os seguintes construtores: \begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
...
\coqdocemptyline
\end{coqdoccode}
O construtor \coqdocvar{pterm\_bvar} é utilizado para representar as variáveis ligadas, \coqdocvar{pterm\_fvar} para variáveis livres, \coqdocvar{pterm\_app} é o construtor de uma aplicação, \coqdocvar{pterm\_abs} é o construtor de abstrações e \coqdocvar{pterm\_labs} é o construtor de abstrações marcadas. \begin{coqdoccode}
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
...
seja, abertura de variáveis é dada abaixo:\begin{coqdoccode}
\coqdocemptyline
\coqdocnoindent
...
\coqdocemptyline
\coqdocnoindent
\coqdockw{Definition} \coqdocvar{body} \coqdocvar{t} := \coqdoctac{\ensuremath{\exists}} \coqdocvar{L}, \coqdockw{\ensuremath{\forall}} \coqdocvar{x}, \coqdocvar{x} \symbol{92}\coqdocvar{notin} \coqdocvar{L} \ensuremath{\rightarrow} \coqdocvar{term} (\coqdocvar{t} \^{} \coqdocvar{x}).\coqdoceol
\coqdocemptyline
\end{coqdoccode}
...
\coqdocemptyline
\coqdocnoindent
\coqdockw{Definition} \coqdocvar{lbody} \coqdocvar{t} := \coqdoctac{\ensuremath{\exists}} \coqdocvar{L}, \coqdockw{\ensuremath{\forall}} \coqdocvar{x}, \coqdocvar{x} \symbol{92}\coqdocvar{notin} \coqdocvar{L} \ensuremath{\rightarrow} \coqdocvar{lterm} (\coqdocvar{t} \^{} \coqdocvar{x}).\coqdoceol
\coqdocemptyline
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
...
os termos resultantes de uma redução em uma abstração, estão relacionados com os termos que possuiam a abstração. \begin{coqdoccode}
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
...
Na prova de barendregt para a confluência do cálculo lambda também é muito importante provar a propriedade de que, se um termo M reduz para um termo N, então o resultado da
 aplicação da função phi em M, também reduz para o resultado da aplicação da função phi no termo N.\begin{coqdoccode}
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
...
o sucesso em realizar esta prova provavelmente depende um caminho intermediário entre essas duas opções. \begin{coqdoccode}
\coqdocemptyline
\coqdocemptyline
\end{coqdoccode}
